...
// --- atomic write helpers for filters.json ---
// serialize writes using a promise chain to avoid concurrent write races
let filtersWriteChain = Promise.resolve();
...
function enqueueFiltersWrite(obj){
  // enqueue a write so writes happen serially
  filtersWriteChain = filtersWriteChain.then(() => atomicWriteJson(filtersPath, obj)).catch(err => {
    console.error('Queued filters write failed:', err && err.message);
  });
  return filtersWriteChain;
}

// ensure filters file exists (create atomically if missing)
try{
  if(!fs.existsSync(filtersPath)){
    // create via temp file + rename to avoid partial files
    const initObj = { rules: [] };
    const tmp = `${filtersPath}.tmp-init-${Date.now()}`;
    fs.writeFileSync(tmp, JSON.stringify(initObj, null, 2), 'utf8');
    fs.renameSync(tmp, filtersPath);
  }
...
app.get('/api/filters', async (req, res) => {
  try{
    const raw = await fs.promises.readFile(filtersPath, 'utf8');
    try{
      const j = JSON.parse(raw || '{}');
      return res.json({ success:true, data: j });
    }catch(parseErr){
      // corrupted JSON -> attempt to back it up and replace with a fresh store
      console.error('Invalid JSON in filters store, backing up corrupted file:', parseErr && parseErr.message);
      try{
        const corruptPath = `${filtersPath}.corrupt-${Date.now()}`;
        await fs.promises.rename(filtersPath, corruptPath);
        const fresh = { rules: [] };
        await atomicWriteJson(filtersPath, fresh);
        return res.json({ success:true, data: fresh, recoveredFrom: corruptPath });
      }catch(recoverErr){
        console.error('Failed to recover filters.json:', recoverErr && recoverErr.message);
        return res.status(500).json({ success:false, error: 'Invalid JSON in filters store and recovery failed' });
      }
    }
  }catch(err){
    console.error('Error reading filters.json', err && err.message);
...
app.post('/api/filters', async (req, res) => {
  ...
  try{
    try{
      const count = Array.isArray(payload.rules) ? payload.rules.length : 0;
      console.log(`💾 /api/filters POST - saving ${count} rule(s) at ${new Date().toISOString()}`);
    }catch(e){}
    await enqueueFiltersWrite(payload);
    return res.json({ success:true, data: payload });
  }catch(err){
    console.error('Error writing filters.json', err && err.message);
    return res.status(500).json({ success:false, error: err && err.message });
  }
});
...
app.delete('/api/filters/:id', async (req, res) => {
  ...
  try{
    const raw = await fs.promises.readFile(filtersPath, 'utf8');
    let store = { rules: [] };
    try{ store = JSON.parse(raw||'{}'); }catch(e){ /* ignore and treat as empty */ }
    if(!Array.isArray(store.rules)) store.rules = [];
    const before = store.rules.length;
    console.log(`🗑️ /api/filters DELETE - requested id=${id} (store had ${before} rules)`);
    store.rules = store.rules.filter(r => String(r._id || r.id || r.id_str || '') !== String(id));
    const after = store.rules.length;
    const removed = before - after;
    console.log(`🗑️ /api/filters DELETE - removed ${removed} rule(s); writing updated store with ${after} rules`);
    await enqueueFiltersWrite(store);
    return res.json({ success:true, removed: removed, data: store });
  }catch(err){
    console.error('Error deleting filter', err && err.message);
    return res.status(500).json({ success:false, error: err && err.message });
  }
});
